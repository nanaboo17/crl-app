...
export default function TerritoryManager() {
  ...
  const filtered = useMemo(() => {
    ...
    return coverage.filter((row) => {
      ...
      if (!q) return true
      const assignedAgent = agents.find((agent) => agent.email.toLowerCase() === row.agent_email?.toLowerCase())
      return [row.territory_code, row.region || '', assignedAgent?.agent_name || '', row.agent_email || '']
        .some((value) => value.toLowerCase().includes(q))
    })
  }, [agents, coverage, search, selectedRegion])
  ...
  return (
    <div className={styles.manager}>
      ...
      <div className={styles.toolbar}>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search territory or agent"
          />
        </div>
      ...
      <div className={styles.summaryLine}>
        ...
        <span>·</span>
        <span><strong>{totals.sites.toLocaleString('id-ID')}</strong> sites</span>
        <span>·</span>
      ...
      {filtered.length === 0 ? (
        <div className={styles.empty}>No territories match the selected filter.</div>
      ) : (
              ...
              <section key={row.territory_code} className={styles.territoryCard}>
                ...
                {assignedAgent && (
                  <p className={styles.agentNote}>
                    {assignedAgent.agent_name} owns all unfinished customers in this territory.
                  </p>
                )}
              </section>
            )
