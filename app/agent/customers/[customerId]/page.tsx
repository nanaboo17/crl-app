...
export default async function AgentCustomerDetailPage({ params }: { params: Promise<{ customerId: string }> }) {
  ...
  const { data: agent } = await supabase.from('agents').select('role, active').eq('email', email).maybeSingle()
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 p-4 sm:p-6 lg:p-8">
      <SuperadminPageHeader breadcrumbs={[{ label: t('agent.customer.breadcrumbAgent'), href: '/agent', icon: UserRound }, { label: t('agent.customer.breadcrumbCustomers'), href: '/agent/customers', icon: Building2 }, { label: customer.customer_name }]} title={customer.customer_name} description={`${customer.customer_id} · ${area}`} />
      ...
      <section aria-label={t('agent.customer.summaryAria')} className="dui-stats dui-stats-vertical sm:dui-stats-horizontal w-full overflow-hidden rounded-box border border-base-300 bg-base-100 shadow-sm">
        <div className="dui-stat"><div className="dui-stat-title">{t('agent.customer.statPriority')}</div><div className={`dui-stat-value ${priority < 3 ? 'text-error' : priority === 3 ? 'text-warning' : ''}`}>{priority ?? '-'}</div>{priority && <div className="dui-stat-desc">{priority === 1 ? t('agent.customer.priorityUrgent') : priority === 2 ? t('agent.customer.priorityUrgent2') : priority === 3 ? t('agent.customer.priorityMedium') : t('agent.customer.priorityLow')}</div>}</div>
        <div className="dui-stat"><div className="dui-stat-title">{t('agent.customer.statDaysToChurn')}</div><div className={`dui-stat-value ${days !== null && days <= 7 ? 'text-error' : ''}`}>{days ?? '-'}</div>{days !== null && <div className="dui-stat-desc">{days <= 0 ? t('agent.customer.churnOverdue') : days <= 7 ? t('agent.customer.churnUrgent') : t('agent.customer.churnOk')}</div>}</div>
        <div className="dui-stat"><div className="dui-stat-title">{t('agent.customer.statVisitStatus')}</div><div className="dui-stat-value text-2xl">{visit ? <CheckCircle2 className="inline-block h-8 w-8 text-success" aria-hidden="true" /> : <MapPin className="inline-block h-8 w-8 text-primary" aria-hidden="true" />}</div><div className="dui-stat-desc">{visit ? t('agent.customer.visited') : t('agent.customer.notVisited')}</div></div>
      </section>

      {visit ? <div className="dui-alert dui-alert-success"><CheckCircle2 className="h-5 w-5 shrink-0" aria-hidden="true" /><span>{t('agent.customer.visitComplete')}{visit.visit_date ? ` · ${new Date(visit.visit_date).toLocaleString('id-ID')}` : ''}{visit.visit_status_kunjungan ? ` · ${visit.visit_status_kunjungan}` : ''}</span></div> : <div className="dui-alert dui-alert-info"><MapPin className="h-5 w-5 shrink-0" aria-hidden="true" /><span>{t('agent.customer.visitTimeDue')}</span></div>}

      <section className={styles.timelineCard} aria-label={tx('Customer timeline', 'Linimasa pelanggan')}>
        <div className={styles.sectionHeading}><BellRing aria-hidden="true" /><div><h2>{tx('Customer timeline', 'Linimasa pelanggan')}</h2><p>{tx('Assignment, pre-visit, visit and follow-up activity in one place.', 'Penugasan, pre-visit, kunjungan, dan tindak lanjut dalam satu tempat.')}</p></div></div>
        <div className={styles.timeline}>{timeline.map((item, index) => <div className={styles.timelineItem} key={`${item.at}-${index}`}><div className={styles.timelineRail}><span className={styles.timelineDot} />{index < timeline.length - 1 && <span className={styles.timelineLine} />}</div><div className={styles.timelineContent}><strong>{item.title}</strong><span>{new Date(item.at).toLocaleString(locale === 'id' ? 'id-ID' : 'en-GB')}</span>{item.detail && <span>{item.detail}</span>}</div></div>)}</div>
      </section>
      ...
      <section aria-label={t('agent.customer.actionsAria')} className="dui-card border border-base-300 bg-base-100 shadow-sm"><div className="dui-card-body">{visit ? <Link href={`/agent/customers/${encodeURIComponent(customer.customer_id)}/visit-result`} className="dui-btn dui-btn-primary w-full"><BadgeCheck className="h-5 w-5" aria-hidden="true" />{t('agent.customer.viewVisitResult')}</Link> : !preVisit ? <Link href={`/agent/customers/${encodeURIComponent(customer.customer_id)}/pre-visit`} className="dui-btn dui-btn-primary w-full"><ClipboardList className="h-5 w-5" aria-hidden="true" />{t('agent.customer.startPreVisit')}</Link> : preVisit.previsit_status !== 'Ready for Visit' && preVisit.previsit_status !== 'Direct Visit' ? <Link href={`/agent/customers/${encodeURIComponent(customer.customer_id)}/pre-visit`} className="dui-btn dui-btn-secondary w-full"><ClipboardList className="h-5 w-5" aria-hidden="true" />{t('agent.customer.continuePreVisit')}</Link> : <><div className="flex items-center gap-2 rounded-box bg-success/10 px-4 py-3 text-sm font-semibold text-success"><CheckCircle2 className="h-5 w-5" aria-hidden="true" />{t('agent.customer.preVisitReady')}</div><Link href={`/agent/customers/${encodeURIComponent(customer.customer_id)}/visit`} className="dui-btn dui-btn-primary w-full"><MapPin className="h-5 w-5" aria-hidden="true" />{t('agent.customer.startVisit')}</Link></>}</div></section>
    </div>
  )
}

function ErrorBlock({ message, backHref, t }: { message: string; backHref: string; t: (key: string, params?: Record<string, string | number>) => string }) { return <div className="mx-auto w-full max-w-4xl p-4 sm:p-6 lg:p-8"><Link href={backHref} className="dui-btn dui-btn-ghost dui-btn-sm mb-4">{t('agent.customer.back')}</Link><div className="dui-alert dui-alert-error" role="alert"><span>{message}</span></div></div> }
function InfoCard({ title, icon: Icon, children }: { title: string; icon: typeof MapPin; children: React.ReactNode }) { return <section aria-label={title} className="dui-card border border-base-300 bg-base-100 shadow-sm"><div className="dui-card-body"><h2 className="flex items-center gap-2 text-base font-bold tracking-tight text-base-content"><Icon className="h-5 w-5 text-primary" aria-hidden="true" />{title}</h2><div className="mt-1">{children}</div></div></section> }
function InfoGrid({ t, items }: { t: (key: string, params?: Record<string, string | number>) => string; items: [string, any][] }) { const addressKey = t('agent.customer.address'); return <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">{items.map(([label, value]) => <div key={label} className={label === addressKey ? 'sm:col-span-2' : ''}><dt className="text-[11px] font-semibold uppercase tracking-wider text-base-content/50">{label}</dt><dd className="mt-1 text-sm font-medium break-words text-base-content">{value === null || value === undefined || value === '' ? '-' : String(value)}</dd></div>)}</dl> }
