...
export default async function AgentCustomerDetailPage({ params }: { params: Promise<{ customerId: string }> }) {
  ...
  const formatWibDateTime = (value: string | null) => value ? `${new Date(value).toLocaleString(locale === 'id' ? 'id-ID' : 'en-GB', { timeZone: TIMEZONE, day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false })} WIB` : '-'
  ...
  const [{ data: preVisits }, { data: visit }, { data: followups }] = await Promise.all([
    supabase.from('pre_visits').select('previsit_id,previsit_status,created_at,updated_at,appointment_date,rescheduled_contact_date,reschedule_date,contact_result').eq('customer_id', decodedCustomerId).eq('agent_email', email).order('created_at', { ascending: false }),
  ...
  const formatDate = (value: string | null) => value ? new Date(value).toLocaleDateString('id-ID', { timeZone: TIMEZONE }) : '-'
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 p-4 sm:p-6 lg:p-8">
      ...
      <section aria-label={t('agent.customer.summaryAria')} className="dui-stats dui-stats-vertical sm:dui-stats-horizontal w-full overflow-hidden rounded-box border border-base-300 bg-base-100 shadow-sm"><div className="dui-stat"><div className="dui-stat-title">{t('agent.customer.statPriority')}</div><div className={`dui-stat-value ${priority < 3 ? 'text-error' : priority === 3 ? 'text-warning' : ''}`}>{priority ?? '-'}</div></div><div className="dui-stat"><div className="dui-stat-title">{t('agent.customer.statDaysToChurn')}</div><div className={`dui-stat-value ${days !== null && days <= 7 ? 'text-error' : ''}`}>{days ?? '-'}</div></div><div className="dui-stat"><div className="dui-stat-title">{t('agent.customer.statVisitStatus')}</div><div className="dui-stat-value text-2xl">{visit ? <CheckCircle2 className="inline-block h-8 w-8 text-success" /> : <MapPin className="inline-block h-8 w-8 text-primary" />}</div><div className="dui-stat-desc">{visit ? t('agent.customer.visited') : t('agent.customer.notVisited')}</div></div></section>
...
function ErrorBlock({ message, backHref, t }: { message: string; backHref: string; t: (key: string, params?: Record<string, string | number>) => string }) { return <div className="mx-auto w-full max-w-4xl p-4 sm:p-6 lg:p-8"><Link href={backHref} className="dui-btn dui-btn-ghost dui-btn-sm mb-4">{t('agent.customer.back')}</Link><div className="dui-alert dui-alert-error"><span>{message}</span></div></div> }
function InfoCard({ title, icon: Icon, children }: { title: string; icon: typeof MapPin; children: React.ReactNode }) { return <section className="dui-card border border-base-300 bg-base-100 shadow-sm"><div className="dui-card-body"><h2 className="flex items-center gap-2 text-base font-bold"><Icon className="h-5 w-5 text-primary" />{title}</h2><div className="mt-1">{children}</div></div></section> }
function InfoGrid({ t, items }: { t: (key: string, params?: Record<string, string | number>) => string; items: [string, any][] }) { const addressKey = t('agent.customer.address'); return <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">{items.map(([label, value]) => <div key={label} className={label === addressKey ? 'sm:col-span-2' : ''}><dt className="text-[11px] font-semibold uppercase tracking-wider text-base-content/50">{label}</dt><dd className="mt-1 text-sm font-medium break-words">{value === null || value === undefined || value === '' ? '-' : String(value)}</dd></div>)}</dl> }
