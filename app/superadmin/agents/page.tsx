  ...
  return (
    <div className={styles.page}>
      <SuperadminPageHeader
        ...
        actions={<Link href="/superadmin/agents/new" className={styles.addButton}><UserPlus aria-hidden="true" className="size-4" />Add Agent</Link>}
      />
      ...
      {agents.length === 0 ? (
        ...
        <>
            ...
            <div className={styles.tableScroll}>
              <table className={styles.table}>
                ...
                <tbody>
                  {agents.map((agent) => (
                    <tr key={agent.email}>
                      <td><Link href={`/superadmin/agents/${encodeURIComponent(agent.email)}`} className={styles.agentLink}><span className={styles.agentName}>{agent.agent_name || '—'}</span><span className={styles.agentEmail}>{agent.email}</span></Link></td>
                      <td className={styles.code}>{agent.sales_code || '—'}</td>
                      <td><span className={styles.badge}>{agent.role}</span></td>
          ...
          <SuperadminPagination page={page} pageSize={PAGE_SIZE} total={count ?? 0} basePath="/superadmin/agents" />
        </>
      )}
    </div>
  )
