...
export default async function AttendancePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const organizations = Array.from(new Set(
    payload.agents.map((agent) => (agent.organization || '').trim()).filter(Boolean)
  ...
  const filteredRows = selectedOrganization
    ? rows.filter((row) => (row.organization || '').trim() === selectedOrganization)
    : rows

  const activeAgents = filteredRows.filter((row) => row.active)
  const present = activeAgents.filter((row) => row.present).length
  const absent = activeAgents.length - present
  const visitCount = filteredRows.reduce((sum, row) => sum + row.visits, 0)
  const preVisitCount = filteredRows.reduce((sum, row) => sum + row.preVisits, 0)
  const todayHref = selectedOrganization
    ? `/superadmin/attendance?organization=${encodeURIComponent(selectedOrganization)}`
  ...
  return (
    <div className="mx-auto grid w-full max-w-7xl gap-5 p-3 sm:gap-6 sm:p-6 lg:p-8">
      ...
      <form className="grid gap-3 rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm sm:flex sm:flex-wrap sm:items-end">
        ...
        <label className="grid w-full gap-1 text-sm font-semibold sm:min-w-56 sm:w-auto">
        ...
        <button type="submit" className="dui-btn dui-btn-primary w-full sm:w-auto">View</button>
        {selectedDate !== today ? <Link href={todayHref} className="dui-btn dui-btn-ghost w-full sm:w-auto">Today</Link> : null}
        {selectedOrganization ? <Link href={`/superadmin/attendance?date=${encodeURIComponent(selectedDate)}`} className="dui-btn dui-btn-ghost w-full sm:w-auto">Clear organization</Link> : null}
      </form>
      ...
      <section className="rounded-2xl border border-base-300 bg-base-100 shadow-sm">
        <div className="border-b border-base-300 p-4">
          <h2 className="font-bold">Agent attendance — {selectedDate}{selectedOrganization ? ` · ${selectedOrganization}` : ''}</h2>
        </div>
