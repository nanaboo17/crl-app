...
export default async function DiagnosticsPage({
  ...
  if (form === 'previsit' || form === 'visit') query = query.eq('form_type', form)
  ...
  return (
    <main className="space-y-5 p-4 sm:p-6 lg:p-8">
      <SuperadminPageHeader
        ...
        description="Detailed Pre-Visit and Visit errors captured from agent devices. Times are shown in WIB."
      />

      <form className="grid gap-3 rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm md:grid-cols-5">
        <label className="dui-fieldset"><span className="dui-fieldset-label">Form</span><select name="form" defaultValue={form} className="dui-select w-full"><option value="">All forms</option><option value="previsit">Pre-Visit</option><option value="visit">Visit</option></select></label>
        ...
        <label className="dui-fieldset"><span className="dui-fieldset-label">Customer ID</span><input name="customer" defaultValue={customer} className="dui-input w-full" placeholder="Customer ID" /></label>
              ...
              <article key={log.log_id} className="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-base-200 pb-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div><div className="font-black">{log.form_type === 'previsit' ? 'Pre-Visit' : 'Visit'} · {log.stage}</div><div className="text-sm text-base-content/60">{fmt(log.created_at)} · {log.agent_email}</div></div>
                  </div>
