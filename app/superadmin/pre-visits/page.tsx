...
import { AlertCircle, ClipboardList, Eye, Inbox, Users } from 'lucide-react'
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.summaryGrid}>
        <div className={styles.summaryCard}><div><div className={styles.summaryLabel}>{t('superadmin.preVisits.totalAgents')}</div><div className={styles.summaryValue}>{totalAgents ?? 0}</div></div><div className={styles.summaryIcon}><Users aria-hidden="true" className="size-5" /></div></div>
        <div className={styles.summaryCard}><div><div className={styles.summaryLabel}>{t('superadmin.preVisits.totalPreVisits')}</div><div className={styles.summaryValue}>{totalPreVisitsResult.count ?? 0}</div></div><div className={styles.summaryIcon}><ClipboardList aria-hidden="true" className="size-5" /></div></div>
      </section>
          ...
          <div className={styles.tableCard}>
            <div className={styles.tableScroll}>
              <table className={styles.table}>
                ...
                <tbody>{agentData.map((agent) => <tr key={agent.email}><td><Link href={`/superadmin/pre-visits/${encodeURIComponent(agent.email)}`} className={styles.agentLink}><span className={styles.agentName}>{agent.agent_name || '—'}</span><span className={styles.agentEmail}>{agent.email}</span></Link></td><td>{agent.sales_code || '—'}</td><td><span className={`${styles.badge} ${agent.active ? styles.active : styles.inactive}`}>{agent.active ? t('superadmin.status.active') : t('superadmin.status.inactive')}</span></td><td className={styles.count}>{agent.previsit_count}</td><td className={styles.actionCell}><Link href={`/superadmin/pre-visits/${encodeURIComponent(agent.email)}`} aria-label={t('superadmin.preVisits.viewAria', { name: agent.agent_name })} title={t('superadmin.preVisits.viewTitle')} className={styles.iconButton}><Eye aria-hidden="true" className="size-4" /></Link></td></tr>)}</tbody>
              </table>
            </div>
          </div>
