...
export default async function AdminPreVisitsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: allAgents, error }, { data: allPreVisits }, totalResult] = await Promise.all([
  ...
  const preVisits = allPreVisits ?? []
  const preVisitMap = new Map<string, typeof preVisits>()
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.summaryGrid} aria-label={tx('Pre-visit summary', 'Ringkasan pra-kunjungan')}><div className={`${styles.summaryCard} ${styles.purple}`}><div><span>{t('superadmin.preVisits.totalAgents')}</span><strong>{(allAgents ?? []).length}</strong></div><div className={styles.summaryIcon}><Users /></div></div><div className={`${styles.summaryCard} ${styles.yellow}`}><div><span>{t('superadmin.preVisits.totalPreVisits')}</span><strong>{totalResult.count ?? 0}</strong></div><div className={styles.summaryIcon}><ClipboardList /></div></div><div className={`${styles.summaryCard} ${styles.green}`}><div><span>{tx('Ready for Visit', 'Siap Dikunjungi')}</span><strong>{readyCount}</strong></div><div className={styles.summaryIcon}><ClipboardCheck /></div></div><div className={`${styles.summaryCard} ${styles.orange}`}><div><span>{tx('Need Follow-up', 'Perlu Tindak Lanjut')}</span><strong>{followUpCount}</strong></div><div className={styles.summaryIcon}><RefreshCcw /></div></div></section>

      <nav className={styles.filterBar} aria-label={tx('Pre-visit filters', 'Filter pra-kunjungan')}>
        {filters.map((item) => <Link key={item.key} href={`/superadmin/pre-visits?filter=${item.key}&page=1`} className={`${styles.filterChip} ${filter === item.key ? styles.filterActive : ''}`}>{item.label}<span>{item.count}</span></Link>)}
      </nav>
