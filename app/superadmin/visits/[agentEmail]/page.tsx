...
export default async function AgentVisitDaysPage({
  ...
  if (!agent) {
    return <main className={styles.page}><div className={styles.errorCard}>{t('superadmin.visits.agentDays.notFound')}</div></main>
  }
  ...
  if (error) {
    return <main className={styles.page}><div className={styles.errorCard}>{error.message}</div></main>
  }
    ...
    <main className={styles.page}>
      ...
      <section className={styles.statsGrid} aria-label={tx('Visit summary', 'Ringkasan kunjungan')}>
        <div className={styles.statCard}>
          <div className={styles.statTop}><span>{t('superadmin.visits.agentDays.totalVisits')}</span><MapPin className={styles.statIcon} aria-hidden="true" /></div>
          <strong>{visits?.length ?? 0}</strong>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statTop}><span>{tx('Active visit days', 'Hari kunjungan aktif')}</span><CalendarDays className={styles.statIcon} aria-hidden="true" /></div>
          <strong>{days.length}</strong>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statTop}><span>{tx('GPS match', 'GPS sesuai')}</span><MapPin className={styles.statIcon} aria-hidden="true" /></div>
          <strong>{gpsMatch}</strong>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statTop}><span>{tx('GPS mismatch', 'GPS tidak sesuai')}</span><MapPin className={styles.statIcon} aria-hidden="true" /></div>
          <strong>{gpsMismatch}</strong>
        </div>
      </section>
      ...
      <section className={styles.historyCard}>
        <div className={styles.sectionHeader}>
          <div>
            <h2>{tx('Visit history by day', 'Riwayat kunjungan per hari')}</h2>
        ...
        <div className={styles.list}>
          {days.length > 0 ? (
            days.map(([date, stats]) => (
              <Link
                key={date}
                href={`/superadmin/visits/${encodeURIComponent(decodedEmail)}/${date}`}
                className={styles.dayCard}
              >
                <div className={styles.dateBlock}>
                  <span className={styles.dateLabel}>
                    {new Date(`${date}T00:00:00`).toLocaleDateString(locale === 'id' ? 'id-ID' : 'en-GB', {
                ...
                <div className={styles.cardRight}>
                  <div className={styles.badges}>
                    <span className={styles.matchBadge}>✓ {stats.match} {tx('match', 'sesuai')}</span>
                    <span className={styles.mismatchBadge}>⚠ {stats.mismatch} {tx('mismatch', 'tidak sesuai')}</span>
                  </div>
