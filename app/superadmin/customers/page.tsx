...
export default async function ManageCustomersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [totalResult, assignedResult, visitedResult, priorityResult] = await Promise.all([
    supabase.from('actionable_customers').select('customer_id', { count: 'exact', head: true }),
    supabase.from('actionable_customers').select('customer_id', { count: 'exact', head: true }).not('agent_email', 'is', null),
    supabase.from('actionable_customers').select('customer_id', { count: 'exact', head: true }).ilike('visit_status', 'visited'),
    supabase.from('actionable_customers').select('customer_id', { count: 'exact', head: true }).in('priority_rank', ['P1', 'P2']),
  ])
  ...
  const { data: customers, error } = await supabase
    .from('actionable_customers')
    .select('customer_id, customer_name, phone_number, outstanding_amount, customer_status, visit_status, agent_email, priority_rank, city, district, household_account_count')
    .order('customer_name')
  ...
  const summaries = [
    { label: tx('Actionable Customers', 'Pelanggan Aktif'), value: total, icon: UsersRound, tone: 'purple' },
    ...
    { label: tx('Priority P1–P2', 'Prioritas P1–P2'), value: priority, icon: AlertCircle, tone: 'yellow' },
  ]
  ...
  return (
    <div className={styles.page}>
      <SuperadminPageHeader
        ...
        description={tx('Only unpaid actionable households are shown. Duplicate billing accounts remain stored for history.', 'Hanya household aktif yang belum bayar yang ditampilkan. Billing account duplikat tetap tersimpan untuk histori.')}
      ...
      <section className={styles.hero}>
        <div>
          ...
          <h2>{tx('Keep every actionable household visible and moving forward.', 'Pastikan setiap household aktif terlihat dan terus bergerak maju.')}</h2>
          <p>{tx('Paid accounts are excluded, while duplicate household billing accounts are collapsed to one actionable customer.', 'Akun yang sudah bayar tidak ditampilkan, sedangkan billing account household duplikat diringkas menjadi satu pelanggan aktif.')}</p>
        </div>
      ...
      {customers.length === 0 ? (
        ...
        <section className={styles.rosterSection}>
          <div className={styles.sectionHeader}>
            <div><h2>{tx('Actionable Customer List', 'Daftar Pelanggan Aktif')}</h2><p>{tx('Open a customer to review details and field history.', 'Buka pelanggan untuk melihat detail dan riwayat lapangan.')}</p></div>
            <span>{total.toLocaleString('id-ID')} {tx('actionable customers', 'pelanggan aktif')}</span>
          </div>
                ...
                <tbody>
                  {customers.map((customer, index) => (
                    <tr key={customer.customer_id}>
                      <td><Link href={`/superadmin/customers/${encodeURIComponent(customer.customer_id)}`} className={styles.customerLink}><span className={`${styles.avatar} ${styles[`avatar${index % 4}`]}`}>{(customer.customer_name || 'C').slice(0, 1).toUpperCase()}</span><span><span className={styles.customerName}>{customer.customer_name || '—'}</span><span className={styles.customerId}>{customer.customer_id}{Number(customer.household_account_count || 1) > 1 ? ` · ${customer.household_account_count} billing accounts` : ''}</span></span></Link></td>
                      ...
                      <td><div className={styles.statusStack}><span className={styles.badge}>{customer.customer_status || '—'}</span>{customer.priority_rank ? <span className={styles.priority}>{customer.priority_rank}</span> : null}</div></td>
          ...
          <div className={styles.mobileList}>
            {customers.map((customer, index) => (
              <article key={customer.customer_id} className={styles.mobileCard}>
                <div className={styles.mobileTop}><div className={styles.mobileIdentity}><span className={`${styles.avatar} ${styles[`avatar${index % 4}`]}`}>{(customer.customer_name || 'C').slice(0, 1).toUpperCase()}</span><div><div className={styles.customerName}>{customer.customer_name || '—'}</div><div className={styles.customerId}>{customer.customer_id}{Number(customer.household_account_count || 1) > 1 ? ` · ${customer.household_account_count} billing accounts` : ''}</div></div></div><div className={styles.statusStack}><span className={styles.badge}>{customer.customer_status || '—'}</span>{customer.priority_rank ? <span className={styles.priority}>{customer.priority_rank}</span> : null}</div></div>
