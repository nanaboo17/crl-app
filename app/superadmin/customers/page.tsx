...
export default async function ManageCustomersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const loadPage = async (page: number): Promise<CustomerCache> => {
    const makeFilteredQuery = (count = false) => {
      let query = supabase.from('customers').select(
        count ? 'customer_id' : 'customer_id, customer_name, phone_number, outstanding_amount, customer_status, visit_status, agent_email, priority_rank, city, district, payment_status, assign_status',
        count ? { count: 'exact', head: true } : undefined
      )
      if (status === 'unassigned') query = query.is('agent_email', null).neq('payment_status', 'paid')
      if (status === 'assigned') query = query.not('agent_email', 'is', null).neq('payment_status', 'paid')
      if (status === 'visited') query = query.or('customer_status.ilike.%visited%,visit_status.ilike.visited')
      if (status === 'paid') query = query.or('payment_status.eq.paid,customer_status.ilike.paid')
      return query
    }
    ...
    const dataResult = await makeFilteredQuery(false)
      .order('customer_name')
    ...
    return {
      ...
      customers: (dataResult.data ?? []) as CustomerRow[],
    }
  }

  let payload: CustomerCache
  try {
    payload = await cacheGetOrSet(`crl:superadmin:customers:v1:${status}:${requestedPage}`, CACHE_TTL, () => loadPage(requestedPage))
  } catch (error) {
